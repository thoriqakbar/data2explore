...
## 018 — Tabbed Results with Performance metrics and distribution charts

**Date:** 2026-03
**Context:** The Results page showed everything in one long scroll — profile, summary, HFC checks, enumerator patterns. Supervisors need operational metrics (daily completions, enumerator productivity), while RAs need data exploration (distributions, percentiles). Mixing both in one view overwhelmed both personas.
**Decision:** Split Results into three tabs: (1) Survey Performance — daily completions chart, duration distribution, enumerator stats table with flag counts; (2) Summary & Distributions — profile overview, summary stats with click-to-expand histograms and percentiles; (3) Data Quality — all existing check results (delta cards, filters, Problem Review, All Flags, Enumerator Patterns, export). Added a new `performance` engine subcommand and extended `summarize` with histogram/percentile data. Installed Recharts for charting.
**Reasoning:** Tabs separate concerns by persona: supervisors focus on Performance + Quality, RAs on Summary + Quality. Filter state is tab-local so switching tabs doesn't reset filters. The `performance` subcommand runs after `check` so it can read `summary.json` for per-enumerator flag counts. Histograms and percentiles are computed inside `summarize` to avoid re-reading the dataset. Recharts was chosen over lightweight alternatives because bundle size doesn't matter for a desktop app and it provides the composable chart API needed for dual-axis charts. The daily-by-enumerator breakdown is capped at 30 enumerators to prevent chart overload.
